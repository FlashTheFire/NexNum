...
class TestPhase4DeviceScorer(unittest.TestCase):
    ...
    def test_02_battery_health_scoring(self):
        ...
        c_high = loop.run_until_complete(DeviceScorer.score_sim_node(None, high_batt, "tg", "u1", self.now))
        c_low = loop.run_until_complete(DeviceScorer.score_sim_node(None, low_batt, "tg", "u1", self.now))
    ...
    def test_03_offline_exclusion(self):
        ...
        c_offline = loop.run_until_complete(DeviceScorer.score_sim_node(None, offline_node, "tg", "u1", self.now))
