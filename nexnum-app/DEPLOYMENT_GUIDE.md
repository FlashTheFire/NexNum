...
**Domain:** `nexnum.in` (you own) with sub‑domain `socket.nexnum.in`  
...
- **Domain `nexnum.in`** registered (Hostinger or any registrar).  
...
In Hostinger (or your DNS provider) → **Manage DNS records** for `nexnum.in`:

| Type | Name (Host) | Value (points to) | TTL |
|------|-------------|-------------------|-----|
| A    | `@` (or leave blank) | `<ELASTIC_IP>` | 300 s |
| A    | `socket`   | `<ELASTIC_IP>` | 300 s |
| CNAME| `www`      | `nexnum.in`        | 300 s *(optional)* |
...
| `NEXT_PUBLIC_APP_URL` | `https://nexnum.in` | Must match the domain you will use for the frontend |
| `NEXTAUTH_URL` | `https://nexnum.in` | Same as above (Needed for NextAuth) |
| `NEXT_PUBLIC_SOCKET_URL` | `wss://socket.nexnum.in` | **Important:** use the **socket** sub‑domain; keep the `wss://` scheme |
| `NODE_ENV` | `production` | |
...
NEXT_PUBLIC_APP_URL=https://nexnum.in
NEXTAUTH_URL=https://nexnum.in
NEXT_PUBLIC_SOCKET_URL=wss://socket.nexnum.in
NODE_ENV=production
...
2. **nexnum-app**  
   ...
   - **Domain:** `nexnum.in`  
   - (Optionally add `www.nexnum.in` as an alias – Coolify will automatically forward it to the same container.)  
   - Save. Traefik will issue a Let’s Encrypt certificate for `nexnum.in` (and `www.nexnum.in` if added) and route HTTP(S) traffic to port 3000 inside the container.

3. **nexnum-socket**  
   - Domain tab → **Domain:** `socket.nexnum.in`  
...
|-------|------------------|-----------------|
| **Frontend loads** | Open browser → `https://nexnum.in` | Next.js UI loads (landing page / login). |
| **Health endpoint** | `curl -s https://nexnum.in/api/health` | `{"status":"ok"}` |
| **Socket health** | `curl -s https://socket.nexnum.in/api/health` | `{"status":"ok"}` (or similar) |
| **Socket.IO connection** | Open browser devtools → Network → WS → should see a connection attempt to `wss://socket.nexnum.in` (or a 400 if not yet connected via JS – that’s expected). |
...
1. **Uptime Kuma** (external monitoring)  
   ...
   - Add two monitors: `https://nexnum.in` and `https://socket.nexnum.in`.  
...
3. **Grafana + Prometheus** (deeper metrics)  
   - Deploy Prometheus (scrape `https://nexnum.in/api/metrics`) and Grafana via Coolify’s custom service or Docker compose.  
...
| **Socket client fails to connect (websocket error)** | Wrong `NEXT_PUBLIC_SOCKET_URL` or socket service unhealthy | Verify URL matches exactly `wss://socket.nexnum.in` (no trailing slash). Check `nexnum-socket` logs for startup errors. Ensure healthcheck passes. |
...
| **Login/NextAuth loop** | `NEXTAUTH_URL` or `NEXT_PUBLIC_APP_URL` mismatch with actual domain | Both must be `https://nexnum.in` (or `http://` if you deliberately disabled TLS). Also ensure cookies are set with correct domain – NextAuth handles this automatically if URLs match. |
...
[ ] DNS: A @ → <EIP>, A socket → <EIP>, (optional) CNAME www → nexnum.in.
...
[ ] Set environment variables (secrets via openssl rand -hex 32, URLs pointing to nexnum.in and socket.nexnum.in).
[ ] Map domains: nexnum-app → nexnum.in, nexnum-socket → socket.nexnum.in.
[ ] Deploy → verify https://nexnum.in and https://socket.nexnum.in work, healthchecks ok.
[ ] (Optional) Add Uptime Kuma, set up backups, monitor logs, scale to t3.large if needed.
```

Follow these steps and you’ll have a clean, reproducible NexNum deployment managed entirely from your local Coolify instance, with TLS handled automatically by Traefik and your domain `nexnum.in` fully operational. 🚀
