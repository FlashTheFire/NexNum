        ...
        let filteredUnlistedServiceCount = 0
            ...
            for (const p of prices) {
                try {
                    ...
                    // SOFT FILTER: Log unlisted country/service for observability without discarding valid provider offers
                    ...
                    if (!isValidCty) {
                        ...
                        logger.debug(`[SYNC] Unlisted country code/ID '${countryCode}' (auto-included from price payload)`, {
                            context: 'SYNC',
                            provider: provider.name,
                            country: countryCode,
                            service: p.service
                    ...
                    if (!isValidSvc) {
                        ...
                        logger.debug(`[SYNC] Unlisted service code '${p.service}' (auto-included from price payload)`, {
                            context: 'SYNC',
                            provider: provider.name,
                            country: countryCode,
                            service: p.service
            ...
            for (const [groupKey, items] of groupedPrices.entries()) {
                try {
                    ...
                    // GUARD: Drop unknown or pure numeric country codes (e.g. "9999") if unresolvable to a real country
